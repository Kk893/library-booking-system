...
const MobileHomeHeader = () => {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    navigate(`/libraries?search=${searchTerm}&city=${selectedCity}`);
  };
    ...
    <div className="md:hidden">
      <div className={`px-4 py-5 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        <form onSubmit={handleSearch} className="relative">
          ...
          <button type="submit" className="absolute right-3 top-1/2 transform -translate-y-1/2 text-xs text-red-500">
            Search
          </button>
        </form>
      </div>
