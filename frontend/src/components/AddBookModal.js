...
const AddBookModal = ({ isOpen, onClose, onSuccess, libraryId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({
        ...
        synopsis: '',
        totalCopies: 1,
        borrowPolicy: { maxDays: 14, fine: 5, reservationFee: 0 }
      });
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <h2 className="text-lg sm:text-xl font-bold mb-4">Add New Book</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="border-t pt-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs sm:text-sm text-gray-600 mb-1">Max Days</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-xs sm:text-sm text-gray-600 mb-1">Fine (₹/day)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-xs sm:text-sm text-gray-600 mb-1">Reservation Fee (₹)</label>
                <input
                  type="number"
