...
const AddLibraryModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('/api/superadmin/libraries', formData);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto animate-fade-in">
      ...
      <style jsx="true">{`
        ...
        @keyframes scale-from-button {
          from { 
            transform: scale(0.1) translate(-100px, -150px);
            opacity: 0;
        ...
        .animate-scale-in {
          animation: scale-from-button 0.6s cubic-bezier(0.34, 1.56, 0.64, 1);
        }
      `}</style>
    </div>
  );
